    ...
    return (
      <>
        ...
        {acceso.tipo_acceso === 'prueba' && acceso.dias_restantes === 0 && (
          <div className="bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-200 border border-amber-200 dark:border-amber-800 px-4 py-2 text-sm text-center mb-4 rounded-lg flex items-center justify-center gap-2 animate-in slide-in-from-top-2">
            <Clock className="w-4 h-4" />
            <span>Tu periodo de prueba termina pronto.</span>
            <Link href="/suscripcion" className="underline font-bold hover:text-amber-900 dark:hover:text-amber-100">Suscríbete aquí.</Link>
          </div>
        )}
        {children}
      </>
    ...
    <div className="flex items-center justify-center min-h-[60vh] p-4 animate-in zoom-in duration-300">
      <Card className="max-w-md w-full border-destructive/20 shadow-xl overflow-hidden">
        ...
        <CardHeader className="text-center pb-2 pt-8">
          ...
          <CardTitle className="text-2xl font-bold text-foreground">Acceso Restringido</CardTitle>
        </CardHeader>
        <CardContent className="text-center space-y-4">
          <p className="text-muted-foreground">
            Tu <strong>Periodo de Prueba de 24 horas</strong> ha finalizado o no cuentas con una suscripción activa.
          </p>
          <div className="bg-muted p-4 rounded-lg text-sm text-muted-foreground flex items-start gap-3 text-left">
            <AlertTriangle className="w-5 h-5 text-amber-500 flex-shrink-0 mt-0.5" />
            <p>
              Para continuar accediendo a los simulacros y material de estudio, necesitas activar el Plan Premium.
            </p>
          </div>
        </CardContent>
