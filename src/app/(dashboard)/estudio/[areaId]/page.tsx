...
export default function TemasPage() {
  ...
  if (loadingTemas) return <div className="space-y-4"><Skeleton className="h-12 w-1/3" /><Skeleton className="h-64 w-full" /></div>
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        ...
        <div>
          <h2 className="text-2xl font-bold text-foreground">Temas de Estudio</h2>
          <p className="text-muted-foreground">{temas?.length || 0} temas disponibles</p>
        </div>
      </div>
      ...
      <div className="grid gap-4">
        {temas?.map((tema) => {
          ...
          return (
            <Card key={tema.id} className="group hover:border-primary/50 transition-colors">
              <CardContent className="p-0 flex flex-col sm:flex-row sm:items-center justify-between">
                <div className="p-6 flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <h3 className="font-semibold text-lg text-foreground line-clamp-1">{tema.nombre}</h3>
                  </div>
                  ...
                  <div className="pl-11 space-y-3">
                    <div className="flex items-center gap-4 text-sm text-muted-foreground">
                      ...
                      <span className={esCompletado ? 'text-green-600 font-medium' : ''}>
                ...
                <div className="px-6 pb-6 sm:pb-0 sm:pr-6 flex items-center gap-3 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                   <Button variant="ghost" size="sm" className="hidden sm:inline-flex">
                     Leer Teoría
                   </Button>
                   
                   <Link href={`/practica/${tema.id}`} className="w-full sm:w-auto">
                    <Button className={cn("w-full sm:w-auto", esCompletado ? "bg-green-600 hover:bg-green-700" : "")}>
